...
for model_tag in TEST_URLS:
    ...
    json_filepath = ut.grab_file_url(
        json_url, appname='wbia_2d_orientation', check_hash=False
    )
...
for dbdir, desired_viewpoint, use_parts, reviewed_only, test_only in db_value_list:
    ...
    for species in keep_dict:
        for viewpoint in keep_dict[species]:
            ...
            print(
                species, viewpoint, len(aids), len(pids), len(nids),
            )
            ...
            gids_ = ibs_dst.add_images(
                gpath_list, ensure_loadable=False, ensure_exif=False
            )
